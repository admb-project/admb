...
char df1fun_notice[50]="copyright (c) 2006 otter research ltd";
...
df1_one_matrix choleski_decomp(const df1_one_matrix& MM)
{
  ...
  if (rmin !=1 || cmin !=1)
  {
    cerr << "minimum row and column inidices must equal 1 in "
      "df1_one_matrix choleski_decomp(const df1_one_atrix& MM)"
         << endl;
    ad_exit(1);
  }
  if (rmax !=cmax)
  {
    cerr << "Error in df1_one matrix choleski_decomp(const df1_one_matrix& MM)"
...
df1_one_variable& df1_one_variable::operator *= (const df1_one_variable& y)
{
  *(this->get_udot()) = *(this->get_u()) * *y.get_udot()
                 + *(this->get_udot()) * *y.get_u();
...
df1_one_variable& df1_one_variable::operator /= (const df1_one_variable& y)
{
  ...
  *(this->get_udot()) = *(this->get_udot()) * yinv
                 - *(this->get_u()) * square(yinv) * *y.get_udot();
