 ...
 */
  ...
  dvar_vector my_eigenvalues(_CONST dvar_matrix& m)
  {
    ...
    {
      cerr << "error -- non square matrix passed to dvector eigenvalues(_CONST dmatrix& m)\n";
      ad_exit(1);
    }
  ...
  dvar_matrix my_eigenvectors(_CONST dvar_matrix& m)
  {
    if (m.rowsize()!=m.colsize())
    {
      cerr << "error -- non square matrix passed to dmatrix eigenvectors(_CONST dmatrix& m)\n";
      ad_exit(1);
    }
  ...
  void eigens(const dvar_matrix& m, const dvar_matrix& _evecs, const dvar_vector& _evals)
  {
    ...
    if ( (m.rowsize()!=evecs.rowsize()) || (m.colsize()!=evecs.colsize()) )
    {
