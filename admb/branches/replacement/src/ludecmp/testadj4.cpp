  ...
  public:
    ...
    cltudecomp(int l,int u) : indx(l,u), indx2(l,u)
    {
      ivector iv(l+1,u);
      iv.fill_seqadd(l,1);
  ...
  public:
    ...
    int indexmin(){return U.indexmin();}
    int indexmax(){return U.indexmax();}
    void ludecomp_pivot_for_adjoint_1(const dmatrix & M,
      const ivector& index2);
    dmatrix ludecomp_pivot_for_adjoint_2(const dmatrix & M,
  ...
  static void df_my_ln_det(void)
  {
    ...
    ivector index2=restore_ivector_value(vpos);
    dvar_matrix_position Mpos=restore_dvar_matrix_position();
    dmatrix M=restore_dvar_matrix_value(Mpos);
    ...
    int mmin=M.indexmin();
    int mmax=M.indexmax();
    cltudecomp dfclu1(mmin,mmax);
    cltudecomp_for_adjoint clu1(mmin,mmax,3,2);
    clu1.ludecomp_pivot_for_adjoint_1(M,index2);
    ...
    dmatrix dfM=clu1.ludecomp_pivot_for_adjoint_2(M,dfclu1,index2);
  ...
  // *************************************************************8
  // *************************************************************8
  void cltudecomp_for_adjoint::ludecomp_pivot_for_adjoint_1(const dmatrix & M,
    const ivector& index_2)
  {
    int i,j;
    int mmin=indexmin();
    int mmax=indexmax();
  ...
  dmatrix cltudecomp_for_adjoint::
    ludecomp_pivot_for_adjoint_2(const dmatrix & _M,
    const cltudecomp & _dfclu,const ivector& _index2)
  {
    ADUNCONST(dmatrix,M)
    ADUNCONST(cltudecomp,dfclu)
    ADUNCONST(ivector,index2)
    int i,j;
    int mmin=indexmin();
    int mmax=indexmax();
    cltudecomp_for_adjoint & clu=*this;
