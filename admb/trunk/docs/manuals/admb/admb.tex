...
independent parameters on a 1000 MH Pentium III, and more on more powerful
...
$P=(p_{ij})$\footnote{{Hamilton seems to index his matrices with the column
    ...
    $p_{ij}$ may correspond to our $p_{ji}$.}} If we are in state~$j$ at
time~$t$, the probability of being in state~$i$ at time~$t+1$ is~$p_{ij}$.
...
temporary files. \X{\fontindexentry{tt}{ GLOBALS\_SECTION}}
\begin{lstlisting}
GLOBALS_SECTION
  #include <admodel.h>
...
\X{\fontindexentry{tt}{INITIALIZATION\_SECTION}} \X{3-dimensional arrays}
