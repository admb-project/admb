...
% Make a table with days of the week followed by up to 10 columns of numbers vertically aligned on their decimal points.  Wanted it in place, so don't use the table environment.
...
that use finite difference approximations.
...
amount of time needed to calculate the function itself. With \scAD, 
the amount of calculation is less than 5 times the amount of calculation 
needed to calculate the function itself \cite{griewankcorliss1991}.   
In particular, the computing time does not depend on 
the number of independent variables.  This makes
\scAD\ particularly attractive for calculating derivatives for
...
Any experienced C programmer can learn to use \scAD\ in a few hours. While it
is necessary to have a \cplus\ compiler to use \scAD,  only a small
...
a superset of C, so valid \textsc{ansi}-compliant C code needs only a very small
amount of modification to be converted to valid \cplus\ code.

We feel that the best way to get a feel for how \scAD\ is used is
to study a series of examples. These examples have been chosen to
illustrate the use of \scAD\ on problems in different fields from
...
of temporary storage. Consider the fact that inverting a 100-by-100
...
\texttt{independent\_variables}.\X{independent variables} 
...
have been finished.\XX{minimization}{of a function}\X{gradient\_structure@\texttt{gradient\_structure}} 
\begin{lstlisting}
#include <math.h>
#include <fvar.hpp>
...
a warning message
\XX{warning messages}{temporary used for type \texttt{xxx} in function \texttt{yyy}}
\begin{lstlisting}
...
\end{lstlisting}
\X{BEGIN\_MINIMIZATION@\texttt{BEGIN\_MINIMIZATION}}\X{END\_MINIMIZATION@\texttt{END\_MINIMIZATION}}
...
objects. \X{dvector@\texttt{dvector}} \X{dvar\_vector@\texttt{dvar\_vector}} \X{dmatrix@\texttt{dmatrix}} \X{dvar\_matrix@\texttt{dvar\_matrix}} 
...
func(v);
\end{lstlisting}
\XX{constructor}{copy initializer}\X{\fontindexentry{tt}{dvariable(dvariable\&)}}
...
\X{safe arrays} \XX{maximum size}{of a \texttt{dvar\_vector}}
...
data it will contain: \XX{constructor}{\fontindexentry{tt}{dvector}}
\begin{lstlisting}
...
% intermediate calculations. If \texttt{a} and \texttt{b} are \texttt{dvariables}
% then the operation \texttt{a*b} is stored in a temporary variable.
...
\XX{constructor}{\fontindexentry{tt}{d3array}}
\XX{slice}{of a \texttt{d3array}}
...
\XX{vector}{sum over the elements of}\XX{\fontindexentry{tt}{sum}}{operation on a vector}
\begin{lstlisting}
...
\XX{multivariate normal distribution}{calculation of the log-likelihood function for}
\begin{lstlisting}
dvariable ln_det;
dvariable sign;
dvariable l;
...
\texttt{n},  \texttt{m},$\ldots$ The number of elements in the string must match the size of the vector.}
...
\end{lstlisting}
\afterlistingthing{fills the $\texttt{j}^\textrm{th}$ column of a matrix with a vector.}

\begin{lstlisting}
void matrix::rowfill(int& i,vector&)
\end{lstlisting}
\afterlistingthing{fills the $\texttt{i}^\textrm{th}$ row of a matrix with a vector.}
...
\XX{\fontindexentry{tt}{fill\_multinomial}}{filling a vector with random numbers} 
\begin{lstlisting}
void vector::fill_multinomial(long int& n, dvector& p)
\end{lstlisting}
\afterlistingthing{fills a vector with a sequence random numbers from
a multinomial distribution. The parameter~\texttt{p} is a \texttt{dvector} such that
\texttt{p[}$i$\texttt{]} is the probability of returning $i$. The elements of \texttt{p}
must sum to $1$.}
...
While the function \texttt{sfabs} has been set to agree with the absolute
value function \texttt{fabs} for all values greater than 0.001, it
is a simple matter to rescale the function to 
...
\begin{figure}[h]
 ...
 % \vbox{\hsize=.95\textwidth 
  ...
  \caption{Plot of the smoothed absolute value function \texttt{y=1000*sfabs(x/1000)} and the normal  absolute value function \texttt{y=fabs(x)}.}
  \label{fig:operations-functions-01}
\end{figure}
In Figure~\ref{fig:operations-functions-01},
 the function \texttt{sfabs} has been
``magnified'' by a scale factor of 1000, to show how it differs
from the ordinary absolute value function \texttt{fabs}.
...
This chapter is devoted to various matters that enable the
\scAD\ user to create more advanced applications, and to
obtain faster performance from the \scAD\ code created.
Beginning users can skip or briefly skim the topics
contained here.
...
the intermediate data for the derivative calculations.
 Every arithmetic operation
on a \texttt{dvariable} object generates 28--30 bytes of temporary storage.
To see what this means, consider that
to invert an $n\times n$ matrix requires about $n^3$ such operations.  
So, the temporary storage required for inverting
a $100\times100$ matrix would be about 28--30~megabytes.
...
\X{\fontindexentry{tt}{RETURN\_ARRAYS\_INCREMENT}}
\X{\fontindexentry{tt}{RETURN\_ARRAYS\_DECREMENT}}
\XX{error messages}{\fontindexentry{tt}{Overflow in RETURN\_ARRAYS stack. Increase NUM\_RETURN\_ARRAYS}}
\begin{lstlisting}
...
\XX{error messages} {\fontindexentry{tt}{You need to increase the global variable  MAX\_NVAR\_OFFSET to xxx}}
...
\XX{error messages}{\fontindexentry{tt}{Need to increase ARRAY\_MEMBLOCK\_SIZE parameter}}
...
\end{lstlisting}
The access function
\XX{\fontindexentry{tt}{dvariable}}{controlling the maximum number of with \texttt{set\_MAX\_DLINKS}}
\XX{\fontindexentry{tt}{set\_MAX\_DLINKS}}{setting the maximum number of dvariable objects}
\begin{lstlisting}
...
\XX{error messages}{\fontindexentry{tt}{Need to increase the maximum number of dlinks}}
\begin{lstlisting}
...
than \texttt{crit} (default value 1.0\e{-4}).
...
section can skip to Section~\ref{sec:using-routines}, %xx \number\mychapno.2  check this!!
...
\begin{equation}\label{eq:robust-nonlinear-regression-00}% \eqno(\number\mychapno.0)
...
relationship is 
\begin{equation}\label{eq:robust-nonlinear-regression-01} % \eqno(\number\mychapno.1)
...
\begin{equation}\label{eq:robust-nonlinear-regression-02} %\eqno(\number\mychapno.2)
...
\begin{equation}\label{eq:robust-nonlinear-regression-03} %\eqno(\number\mychapno.3)
...
\begin{equation}\label{eq:robust-nonlinear-regression-04} % \eqno(\number\mychapno.4)
...
that minimize
\begin{equation}\label{eq:robust-nonlinear-regression-05} % \eqno(\number\mychapno.5)
...
\begin{equation}\label{eq:robust-nonlinear-regression-06} % \eqno(\number\mychapno.6)
...
\begin{equation}\label{eq:robust-nonlinear-regression-07} % \eqno(\number\mychapno.7)
  \frac{1}{2\sigma} 
    \exp\left|-\frac{u}{\sigma}\right|
\end{equation}
The corresponding log-likelihood function is
\begin{equation}\label{eq:robust-nonlinear-regression-08} % \eqno(\number\mychapno.8)
...
\begin{equation}\label{eq:robust-nonlinear-regression-09} % \eqno(\number\mychapno.9)
...
\begin{equation}\label{eq:robust-nonlinear-regression-10} % \eqno(\number\mychapno.10)
...
\begin{align}\label{eq:robust-nonlinear-regression-11} % \eqno(\number\mychapno.11)
...
\begin{align}\label{eq:robust-nonlinear-regression-12} % \eqno(\number\mychapno.12)
\nonumber
...
A small circle ($\circ$) was used to mark the outliers.
                                                                                                           %fig:robust-nonlinear-regression-01
...
\begin{table}[!h]
  ...
  \begin{tabular}{ l   *{3}{ c *{3}{r@{.}l } }     }
    \hline\\[-11pt]
    \hline\\[-9.0pt]
                   &&  \multicolumn{6}{c}{Normal errors}    && \multicolumn{6}{c}{Moderate deviations}    && \multicolumn{6}{c}{Large deviations}  %&\multicolumn{1}{c}{} 
                   \\[3pt]
...
by Figure~\ref{fig:monte-carlo-03}. %{fig:robust-nonlinear-regression-02}. % \number\mychapno.2.
...
\XX{dependent variables}{\fontindexentry{tt}{set\_NUM\_DEPENDENT\_VARIABLES(int i);}}
...
\begin{equation}\label{eq:problems-more-dependent-variable-02} % \eqno(\number\mychapno.2)
...
\begin{equation}\label{eq:problems-more-dependent-variable-03} %\eqno(\number\mychapno.3)
  \delta=-J(X)^{-1}f(X)
\end{equation}
