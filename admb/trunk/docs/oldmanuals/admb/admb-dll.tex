...
\XX{-dll}{command line option}
\XX{command line arguments}{-dll}
...
Objects which are to be passed into or returned from the
...
\XX{DLL}{commands for producing}
\mysection{Compiling the code to produce a DLL}
...
dllwrap -def %1.def --implib lib%1.a --driver-name c++ -o %1.dll  %1.o 
   -Lf:/admodel/lib -ladmod -ladt -lado -lm 

\endexample
\noindent where the symbol %1 (this is a batch file argument which would
...
It is necessary that SPLUS can find the DLL. If there is trouble a full
path name can be used as in
\beginexample
dll.load("c://mydlls//simpdll.dll",symbol="simpdll")
\endexample

\mysection{The splus objects}
...
\XX{-gaussdll}{command line option}
\XX{command line arguments}{-gaussdll}

\XX{DLL}{debugging}
\mysection{Debugging the DLL's}
...
typedef __declspec(dllexport) void _export (*MYPROC)(int *_nvar,double *_x,double *_freturn,char ** sp_options);
...
\mysection{Understanding what is being passed to the DLL}
...
Splus passes objects to the DLL by address, that is it
...
\endexample
\mysection{Passing strings from Splus to a DLL}
...
\mysection{A mixture of two bivariate normal distributions}
...
point from the mixture  with 25% in one component and 75% in the other
...
Ths example can be run by using the following Splus source code.
\beginexample
...
\mysection{Interpretation of the parameter estimates}
