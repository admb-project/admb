/*
 * $Id: df1b2ludcmp.cpp 494 2012-06-13 20:41:16Z johnoel $
 *
...
df1b2matrix inv(const df1b2matrix& aa)
{
   int lb = aa.colmin();
   int ub = aa.colmax();
   if (lb != aa.rowmin() || ub != aa.colmax())
   ...
   df1b2matrix x(lb, ub,lb,ub);
   x.initialize();

   if (ub == lb)
   {
      x(lb,lb) = 1.0 / aa(lb, lb);
      return (x);
   }
