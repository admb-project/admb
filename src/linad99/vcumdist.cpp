...
dvariable inv_cumd_norm_inner(const prevariable& x)
{
 ...
 RETURN_ARRAYS_INCREMENT();
  ...
  if (x<=0.5)
  {
    ...
    RETURN_ARRAYS_DECREMENT();
    gradient_structure::GRAD_STACK1->RETURN_PTR->v->x=pp;
    gradient_structure::GRAD_STACK1->set_gradient_stack(default_evaluation,
       &(gradient_structure::GRAD_STACK1->RETURN_PTR->v->x), &(x.v->x),dfx);
...
prevariable& cumd_norm(const prevariable& _x)
{
 if (++gradient_structure::GRAD_STACK1->RETURN_PTR > gradient_structure::GRAD_STACK1->MAX_RETURN)
  ...
  if (x>=0)
  {
    ...
    double z=1.0+tmp1*y;
    gradient_structure::GRAD_STACK1->RETURN_PTR->v->x=z;
    ...
    gradient_structure::GRAD_STACK1->set_gradient_stack(default_evaluation,
       &(gradient_structure::GRAD_STACK1->RETURN_PTR->v->x), &(_x.v->x), dfx);
  }
  else
  {
    double w=-x;
