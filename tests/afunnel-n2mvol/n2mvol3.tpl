...
PROCEDURE_SECTION
  ...
  dvar_vector results;
  results = funnel([](const int i, const int nobs, dvar_vector& w, dvar_matrix& h_mean, dmatrix& Y, dvar_vector& delta, dvar_vector& lambda, dvar_vector& lambda2, const int ndim, dvar_matrix& Z, dvar3_array& h_var, dvar_matrix& Rinv, dvar_matrix& S, dvar_matrix& R, dvar_vector& w1, dvar_vector& lmin, bool active_lambda2)
  {
    dvariable result;

    dvar_vector tmp(1,ndim);
    dvar_matrix sh(1,ndim,1,ndim);
    dvar_vector tmean=update_the_means2(w,h_mean(i-1),Y(i-1), delta, lambda, lambda2, active_lambda2);
    dvar_matrix v=update_the_variances2(h_var(i-1), ndim, delta, Z);
    ...
    {
      xfp12b(tmp, Y(i),tmean,v,gr,sh, ndim, Rinv);
      h=-solve(sh,gr);
      tmp+=h;
    }
    ...
    lmin(i)=fp2(tmp,Y(i),tmean,v,ndim, S, R, w1, lmin);

    int sgn;
    result = lmin(i)+0.5*ln_det(sh,sgn);

    return result;
  }, nobs, w, h_mean, Y, delta, lambda, lambda2, ndim, Z, h_var, Rinv, S, R, w1, lmin, active(lambda2));
  f += sum(results);
...
GLOBALS_SECTION
  ...
  dvar_vector update_the_means2(dvar_vector& w,dvar_vector& m,dvector& e, dvar_vector& delta, dvar_vector& lambda, dvar_vector& lambda2, bool active_lambda2)
  {
    dvar_vector tmp= w+elem_prod(delta,m)+elem_prod(lambda,e);
    if (active_lambda2)
      tmp+=elem_prod(lambda2,fabs(e)); 
  ...
  void xfp12b(dvar_vector& h, dvector& y,dvar_vector& m,dvar_matrix& v, dvar_vector gr,dvar_matrix& hess, const int ndim, dvar_matrix& Rinv)
  {
    dvar_vector ehinv=exp(-.5*h);
  ...
  dvar_vector funnel(dvariable (*func)(const int i, const int nobs, dvar_vector& w, dvar_matrix& h_mean, dmatrix& Y, dvar_vector& delta, dvar_vector& lambda, dvar_vector& lambda2, const int ndim, dvar_matrix& Z, dvar3_array& h_var, dvar_matrix& Rinv, dvar_matrix& S, dvar_matrix& R, dvar_vector& w1, dvar_vector& lmin, bool active_lambda2), const int nobs, dvar_vector& w, dvar_matrix& h_mean, dmatrix& Y, dvar_vector& delta, dvar_vector& lambda, dvar_vector& lambda2, const int ndim, dvar_matrix& Z, dvar3_array& h_var, dvar_matrix& Rinv, dvar_matrix& S, dvar_matrix& R, dvar_vector& w1, dvar_vector& lmin, bool active_lambda2)
  {
    ...
    for (int i=2;i<=nobs;i++)
    {
      results(i) = func(i, nobs, w, h_mean, Y, delta, lambda, lambda2, ndim, Z, h_var, Rinv, S, R, w1, lmin, active_lambda2);
    }
    return results;
  }
