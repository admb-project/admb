...
GLOBALS_SECTION
  ...
  {
    const size_t df_file_bytes = 50; //gradient_structure::CMPDIF_BUFFER_SIZE
    const size_t dlist_size = 1765; //gradient_structure::MAX_DLINKS
    const unsigned long indvar_list_size = 4;//gradient_structure::MAX_NVAR_OFFSET
    auto start = std::chrono::high_resolution_clock::now();
    global_grad_stack1 = new grad_stack(6500, df_file_bytes, dlist_size, gradient_structure::ARRAY_MEMBLOCK_SIZE, indvar_list_size, gradient_structure::NUM_DEPENDENT_VARIABLES, 1);
    global_grad_stack2 = new grad_stack(6500, df_file_bytes, dlist_size, gradient_structure::ARRAY_MEMBLOCK_SIZE, indvar_list_size, gradient_structure::NUM_DEPENDENT_VARIABLES, 2);
    global_grad_stack3 = new grad_stack(6500, df_file_bytes, dlist_size, gradient_structure::ARRAY_MEMBLOCK_SIZE, indvar_list_size, gradient_structure::NUM_DEPENDENT_VARIABLES, 3);
    global_grad_stack4 = new grad_stack(6500, df_file_bytes, dlist_size, gradient_structure::ARRAY_MEMBLOCK_SIZE, indvar_list_size, gradient_structure::NUM_DEPENDENT_VARIABLES, 4);
    auto finish = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = finish - start;
    ...
    for (int i = min; i < max; i += 4)
    {
      ...
      std::pair<double, dvector> p1 = f.get();
