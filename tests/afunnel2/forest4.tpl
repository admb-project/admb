...
PROCEDURE_SECTION
  ...
  sigma=mfexp(log_sigma);
   ...
   {
     //a_index=i;
     S(i) = funnel([](const dvariable& tau, const dvariable& nu, const dvariable& sigma, const dvariable& beta, const double ai, const int nsteps)
     {
       dvariable Integral;
...
GLOBALS_SECTION
  ...
  dvariable funnel(
    ...
    const dvariable& tau, const dvariable& nu, const dvariable& sigma, const dvariable& beta, const double ai, const int nsteps)
  {
    std::future<std::pair<double, dvector>> f = afunnel(func, tau, nu, sigma, beta, ai, nsteps);
    std::pair<double, dvector> p = f.get();

    dvariable result;
    result = to_dvariable(p, tau, nu, sigma, beta);
    return result;
  }
