...
PRELIMINARY_CALCS_SECTION
  ...
  global_grad_stack6 = new grad_stack(10000, 10);
...
FINAL_SECTION
  ...
  delete global_grad_stack6;
  global_grad_stack6 = nullptr;
GLOBALS_SECTION
  #include <fvar.hpp>
  #include <admodel.h>
  #include <future>
  ...
  grad_stack* global_grad_stack6 = nullptr;
    ...
    const dvariable& tau, const dvariable& nu, const dvariable& sigma, const dvariable& beta, const dvector& a, const int nsteps)
  {
    ...
    std::future<std::vector<std::pair<double, dvector>>> f = 
      afunnel(func, tau, nu, sigma, beta, a, 1, 2, nsteps, global_grad_stack);
    std::future<std::vector<std::pair<double, dvector>>> f2 = 
      afunnel(func, tau, nu, sigma, beta, a, 3, 4, nsteps, global_grad_stack2);
    std::future<std::vector<std::pair<double, dvector>>> f3 = 
      afunnel(func, tau, nu, sigma, beta, a, 5, 6, nsteps, global_grad_stack3);
    std::future<std::vector<std::pair<double, dvector>>> f4 = 
      afunnel(func, tau, nu, sigma, beta, a, 7, 8, nsteps, global_grad_stack4);
    std::future<std::vector<std::pair<double, dvector>>> f5 = 
      afunnel(func, tau, nu, sigma, beta, a, 9, 10, nsteps, global_grad_stack5);
    std::future<std::vector<std::pair<double, dvector>>> f6 = 
      afunnel(func, tau, nu, sigma, beta, a, 11, 13, nsteps, global_grad_stack6);
    ...
    std::vector<std::pair<double, dvector>> pairs6 = f6.get();
    ...
    for (int i = 11; i <= 13; ++i)
    {
      results(i) = to_dvariable(pairs6[index5], tau, nu, sigma, beta);
      ++index5;
    }
