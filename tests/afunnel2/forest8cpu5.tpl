...
PRELIMINARY_CALCS_SECTION
  auto start = std::chrono::high_resolution_clock::now();
  global_grad_stack1 = new grad_stack(10000, 1);
  global_grad_stack2 = new grad_stack(10000, 2);
  global_grad_stack3 = new grad_stack(10000, 3);
  global_grad_stack4 = new grad_stack(10000, 4);
  global_grad_stack5 = new grad_stack(10000, 5);
  auto finish = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> elapsed = finish - start;
...
GLOBALS_SECTION
    ...
    const dvariable& tau, const dvariable& nu, const dvariable& sigma, const dvariable& beta, const dvector& a, const int nsteps)
  {
    ...
    for (int i = min; i < 11; i += 5)
    {
      ...
      std::future<std::pair<double, dvector>> f5 = 
        afunnel(func, tau, nu, sigma, beta, a(i + 4), nsteps, global_grad_stack5);
      ...
      std::pair<double, dvector> p5 = f5.get();
      ...
      results(i + 4) = to_dvariable(p5, tau, nu, sigma, beta);
    }
    {
      int i = 11;
      cout << "i: " << i << endl;
