...
#endif
compute_gradients(
  grad_stack* global_grad_stack,
  const typename parameters<std::function<dvariable(Args...)>>::types& f, Args&&... args)
{
#ifdef DEV
  std::vector<double_and_int*> ptrsb;
  const int total_variablesb = get_ptrs(ptrsb, std::forward<Args>(args)...);
  return std::async(std::launch::async,
    [=](grad_stack* gradstack, const int total_variables,
      const std::vector<double_and_int*>& ptrs)->std::tuple<double, dvector, std::vector<double_and_int*>>
#endif
    {
      ...
      grad_stack* gradstack = global_grad_stack;
#endif
...
/*
  ...
  cout << "===" << endl;
*/
      return std::make_tuple(result_value, g, std::move(ptrs));
#ifdef DEV
    }, global_grad_stack, total_variablesb, ptrsb);
#else
    }
#endif
}
