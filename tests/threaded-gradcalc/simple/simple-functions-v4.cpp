...
dvar_vector compute_gradients(
  std::function<dvariable(const dvariable& b0, const dvariable& b1, double xi)> f, dvariable& b0, dvariable& b1, dvector& x)
{
  ...
  for (int i = x.indexmin(); i <= x.indexmax(); ++i)
  {
    ...
    gradient_structure::GRAD_STACK1->allocate_RETURN_ARRAYS(25, 70);
    {
      ...
      dvariable result;
      result = f(variables(1), variables(2), x(i));
...
{
  return compute_gradients([](const dvariable& b0, const dvariable& b1, double xi)->dvariable
  {
